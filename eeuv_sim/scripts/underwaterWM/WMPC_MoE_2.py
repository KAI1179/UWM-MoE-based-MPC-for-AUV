...
# 导入世界模型 & 标准化工具
from worldmodel_MoE_3reg import WMConfigMoE3 as WMConfig, MoEWorldModel3 as ROVGRUModel, rollout  # type: ignore
from utils import quat_normalize_np  # type: ignore
# ---------- MoE standardizer 适配器（替换旧 Standardizer） ----------
class Standardizer:
    # 兼容旧接口的适配器：
    # - 通过 npz 文件加载 {mean, std, u_mean, u_std}
    # - 暴露属性：x_mean, x_std, u_mean, u_std（np.ndarray, float32）
    # - 方法：apply_x_np(X)  -> 标准化状态（广播支持 (N,13) 或 (1,13)）
    def __init__(self, x_mean, x_std, u_mean, u_std):
        self.x_mean = x_mean.astype(np.float32)
        self.x_std  = np.maximum(x_std.astype(np.float32), 1e-6)
        self.u_mean = u_mean.astype(np.float32)
        self.u_std  = np.maximum(u_std.astype(np.float32), 1e-6)

    @staticmethod
    def load(path: str) -> "Standardizer":
        arr = np.load(path)
        # 兼容常见命名
        mean   = arr.get("mean")   if "mean"   in arr else arr["x_mean"]
        std    = arr.get("std")    if "std"    in arr else arr["x_std"]
        u_mean = arr.get("u_mean") if "u_mean" in arr else arr["u_mean"]
        u_std  = arr.get("u_std")  if "u_std"  in arr else arr["u_std"]
        return Standardizer(mean, std, u_mean, u_std)

    def apply_x_np(self, X):
        return (X - self.x_mean.reshape(1, -1)) / (self.x_std.reshape(1, -1) + 1e-8)
...
class WMPCController(Node):
    ...
    def __init__(
        ...
        # CEM 参数
        self.declare_parameter('pop_size', 256)  ## 128  每轮采样的轨迹规模
        self.declare_parameter('elite_frac', 0.20)  ## 选取前多少百分比作为精英（用来更新高斯分布的均值/方差）
        ...
        self.declare_parameter('w_att', 1.0)           # 姿态（滚转/俯仰）约束权重：惩罚 qx,qy，鼓励无翻滚
        ...
        self.w_att = float(self.get_parameter('w_att').value)
        ...
        self.search_ahead = 80             # 在 [ref_index, ref_index+search_ahead] 窗口内找最近点  ## 80
        self.advance_thresh = 0.3          # 若足够靠近当前参考点则推进索引（米）
        ...
        # self.log_path = os.path.join(results_dir, "wmpc_rov_log_0821_1505.h5")
        # self.log_path = os.path.join(results_dir, "wmpc_rov_log_0825_1944.h5")
        # self.log_path = os.path.join(results_dir, "wmpc_rov_log_0825_1950.h5")
        # self.log_path = os.path.join(results_dir, "wmpc_MoE_rov_log_0902_2050.h5")
        self.log_path = os.path.join(results_dir, "wmpc_MoE_rov_log_0902_2100.h5")
    ...
    def _solve_cem(self, x0_std: np.ndarray, ref_traj: np.ndarray) -> np.ndarray:
        ...
        for _ in range(self.n_iters):
            ...
            with torch.no_grad():
                ...
                # 姿态（滚转/俯仰）成本：只惩罚 qx,qy，使姿态尽量正立（不限制偏航）


                q_pred_std = x_hat[:, 1:, 3:7]  # (B,N,4)


                q_pred = q_pred_std * torch.from_numpy(self.std.x_std[3:7]).to(device) + torch.from_numpy(self.std.x_mean[3:7]).to(device)


                q_norm = torch.linalg.norm(q_pred, dim=-1, keepdim=True) + 1e-8


                q_unit = q_pred / q_norm


                att_cost = (q_unit[..., 1]**2 + q_unit[..., 2]**2).sum(dim=-1)  # (B,)



                total_cost = self.w_pos * pos_cost + self.w_u * u_energy + self.w_du * du_cost + self.w_att * att_cost
    ...
    def _load_world_model(self, ckpt_path: str, std_path: str, device: str):
        ...
        # 若未提供/未找到，尝试在 ckpt 同目录下查找常见命名


        if (not std_path) or (not os.path.isfile(std_path)):


            cand = ["standardizer_moe.npz", "std_stats.npz", "std_state.npz", "standardizer.npz"]


            for name in cand:


                p = os.path.join(os.path.dirname(ckpt_path), name)
...
def parse_cli_args():
    ...
    parser.add_argument("--ckpt", type=str, default='./checkpoints_moe3_3/moe3_epoch120.pt', help="世界模型 checkpoint 路径（.pt）")
    ...
    parser.add_argument("--std", type=str, default='./checkpoints_moe3_3/standardizer_moe.npz', help="standardizer.npz 路径（默认与 ckpt 同目录）")
