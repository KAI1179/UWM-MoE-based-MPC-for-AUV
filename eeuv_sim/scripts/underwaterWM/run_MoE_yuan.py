...
class SimpleStandardizer:
    ...
    def fit_states(self, X):  # X: (N,13)
        mean = X.mean(0)
        std  = X.std(0).clip(min=1e-6)
        # 不动四元数（保持单位）
        mean[3:7] = 0.0
        std[3:7]  = 1.0
    ...
    def transform_states(self, X: torch.Tensor):
        return (X - self.mean) / self.std

    def inverse_states(self, Xn: torch.Tensor):
        return Xn * self.std + self.mean

    def fit_actions(self, U):  # (N,8)
        u_mean = U.mean(0)
        u_std  = U.std(0).clip(min=1e-6)
        self.u_mean = torch.tensor(u_mean, dtype=torch.float32)
        self.u_std  = torch.tensor(u_std,  dtype=torch.float32)

    def transform_actions(self, U: torch.Tensor):
        return (U - self.u_mean) / self.u_std

    def save(self, path):
        np.savez(path,
                 mean=self.mean.cpu().numpy(), std=self.std.cpu().numpy(),
...
def parse_args():
    ...
    p.add_argument("--file_path", type=str,
        ...
        default="/home/xukai/ros2_ws/src/eeuv_sim/scripts/data_collector/data/rov_data_regimes_thrust1data10_10_labeled.hdf5")
    ...
    p.add_argument("--gate_lb", type=float, default=1e-1)  ## 5e-2
