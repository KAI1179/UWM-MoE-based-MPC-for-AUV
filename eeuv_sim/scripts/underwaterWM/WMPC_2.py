...
class WMPCController(Node):
    ...
    def __init__(
        ...
        # CEM 参数
        self.declare_parameter('pop_size', 1024)  ## 128  每轮采样的轨迹规模
        self.declare_parameter('elite_frac', 0.25)  ## 选取前多少百分比作为精英（用来更新高斯分布的均值/方差）
        self.declare_parameter('n_iters', 3)  ## CEM 外层迭代次数。越大越稳，但耗时增加
        self.declare_parameter('init_std', 2.0)        # 初始采样标准差（牛顿） 决定第一轮搜索的“步幅/探索度”。

        # 代价权重
        self.declare_parameter('w_pos', 10.0)           # 位置误差  ## 5.0
        self.declare_parameter('w_u', 1e-1)            # 控制能量  ## 1e-2
        self.declare_parameter('w_du', 1e-1)           # 控制增量  ## 1e-2
