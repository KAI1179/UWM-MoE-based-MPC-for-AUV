...
class WMPCController(Node):
    def __init__(self,
        ...
        self.Q_ang = np.diag([0.5, 0.5, 0.3])  # angular vel
        self.R = 1e-4  # control magnitude per thruster
        self.Rd = 1e-3 * action_smooth_weight  # action smoothness

        # ---- Added (tracking/attitude tuning; minimal invasive) ----
        self.W_tilt = getattr(self, 'W_tilt', 6.0)  # penalize roll/pitch tilt
        self.W_att_smooth = getattr(self, 'W_att_smooth', 2.0)  # penalize quaternion changes
        self.Q_pos_term_scale = getattr(self, 'Q_pos_term_scale', 3.0)  # terminal position cost scale
        # -----------------------------------------------------------
        ...
        self.ref_lookahead = getattr(self, 'ref_lookahead', 2)
    ...
    def _rollout_cost(self, x0_np: np.ndarray, U_seq_np: np.ndarray, ref_idx: int) -> float:
        ...
        for t in range(T):
            ...
            # 取当前姿态四元数 q_t (w,x,y,z)
            q_t = x_t[:, 3:7]  # (P, 4)
            q_t = torch.nn.functional.normalize(q_t, dim=1)

            # ---- (A) 倾斜惩罚：让机体Z轴尽量对齐世界Z轴 ----
            # 四元数转旋转矩阵的 (2,2) 元素等于 cos(tilt)（当 yaw 任意时），
            # 这里计算 R(q_t)[2,2] 并惩罚其偏离 1。
            w, x, y, z = q_t[:, 0], q_t[:, 1], q_t[:, 2], q_t[:, 3]
            R_zz = 1 - 2 * (x * x + y * y)  # 等价于旋转矩阵的 [2,2]
            tilt_cost = self.W_tilt * (1.0 - R_zz) ** 2
            c = c + tilt_cost

            # ---- (B) 姿态平滑：惩罚相邻时刻四元数变化（不惩罚 yaw 绝对值）----
            if t > 0:
                # 与上一步的姿态距离，使用 |dot(q_t, q_prev)| 的单位四元数距离
                dot_q = torch.abs(torch.sum(q_t * q_prev, dim=1))  # 取绝对值以忽略 q 与 -q 等价
                att_smooth = self.W_att_smooth * (1.0 - dot_q) ** 2
                c = c + att_smooth
    ...
    def _plan(self, x0: np.ndarray) -> np.ndarray:
        ...
        #
        # # ---- Added: monotonic reference progress with small lookahead ----
        # if hasattr(self, 'ref_idx'):
        #     ref_start = max(self.ref_idx, ref_start)
        # else:
        #     self.ref_idx = ref_start
        # ref_start = min(ref_start + int(self.ref_lookahead), self.trajectory.shape[0] - self.N)


        i_curr = np.argmin(np.linalg.norm(self.trajectory - self.position, axis=1))
        ref_start = self.trajectory[i_curr: i_curr + self.N]
        if len(ref_start) < self.N:
            ref_start = np.pad(ref_start, ((0, self.N - len(ref_start)), (0, 0)), mode='edge')
