...
class MoEWorldModel(nn.Module):
    ...
    def single_step_loss(self, x: torch.Tensor, u: torch.Tensor, x_next: torch.Tensor,
        ...
        w_mean = w.mean(dim=(0, 1)).clamp_min(1e-8)  # (K,)
        uniform = torch.full_like(w_mean, 1.0 / w_mean.numel()).clamp_min(1e-8)
        log_u = uniform.log()
        lb_loss = F.kl_div(log_u, w_mean, reduction="batchmean")  # KL(w_mean || uniform)
