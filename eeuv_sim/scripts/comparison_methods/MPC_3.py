...
class MPCController(Node):
    def __init__(self, waypoints=[0.0, 0.0], dt=0.1, horizon=10, max_steps=1500):
        ...
        log_path = os.path.join(os.getcwd(), '/home/xukai/ros2_ws/src/eeuv_sim/scripts/comparison_methods/results/mpc_rov_log_4.h5')
    ...
    def solve_mpc(self, position, velocity, ref_traj):
        """
        速度上限的“软约束”版本：
          |v_i| <= v_bound + s_i,  s_i >= 0
        并用大权重惩罚 s_i，保证能解、又尽量不超速。
        其余保持不变（位置代价 self.Q、控制代价 self.R、±20N 输入限、转矩=0）。
        """
        dt = self.timer_period

        # 速度上限（总范数 ~ 1 m/s 的逐轴保守近似）
        v_max = 1.0
        v_bound = v_max / np.sqrt(3.0)

        # 松弛变量的权重（越大越不愿超速；先取 200~1000 量级）
        w_slack = 400.0

        # 变量
        x = cp.Variable((6, self.N + 1))  # [p; v]
        u = cp.Variable((6, self.N))  # [Fx,Fy,Fz,Mx,My,Mz]
        s = cp.Variable((3, self.N), nonneg=True)  # 速度上限的松弛变量 s>=0
        ...
        for k in range(self.N):
            ...
            # 速度上限“软约束” |v| <= v_bound + s
            constraints += [cp.abs(v_k) <= v_bound + s[:, k]]
            # 惩罚松弛（用二次惩罚保持QP）
            cost += w_slack * cp.sum_squares(s[:, k])
        ...
        # 同时接受 OPTIMAL_INACCURATE，避免数值轻微不精确导致“失败”
