...
def make_health_mask_gt(time, regime_id, fault_index, t_fail, n_thrusters=8, delay_s=0.0):
    """
    生成基础 GT 掩码：故障通道在 t >= t_fail + delay_s 置 0，其余为 1。
    非故障工况或 fault_index < 0 时，全 1。
    注意：默认 delay_s=0.0（取消延迟标注）。
    """
    N = len(time)
    ...
    if regime_id == 4 and (fault_index is not None) and (fault_index >= 0) and np.isfinite(t_fail):
        onset = float(t_fail) + float(delay_s)
        idx = int(np.clip(np.searchsorted(time.astype(float), onset, side="left"), 0, N))
        if idx < N:
            gt[idx:, fault_index] = 0.0
    return gt

def apply_min_dwell(binary_series, min_dwell_steps=2):
    """对单个二值序列应用“最小驻留”。"""
    x = binary_series.astype(np.int32).copy()
    N = len(x)
...
def hysteresis_min_dwell_mask(gt_mask, min_dwell_steps=2):
    """对 (N,8) 的 GT 掩码逐通道应用最小驻留。"""
    N, M = gt_mask.shape
...
def make_postfail_mask(time, regime_id, t_fail):
    """step级 post-fail 掩码：工况4且 t>=t_fail 为1，否则0；不受 dwell 影响。"""
    N = len(time)
    m = np.zeros(N, dtype=np.float32)
    if regime_id == 4 and np.isfinite(t_fail) and t_fail >= 0:
        idx = int(np.clip(np.searchsorted(time.astype(float), float(t_fail), side="left"), 0, N))
        if idx < N:
            m[idx:] = 1.0
    return m

def make_fail_gap_mask(time, regime_id, t_fail, fail_gap=0.0):
    """可选：标记 t 在 [t_fail - fail_gap, t_fail + fail_gap] 的步为1，用于训练时屏蔽边界。"""
    N = len(time)
    m = np.zeros(N, dtype=np.float32)
    if fail_gap > 0.0 and regime_id == 4 and np.isfinite(t_fail) and t_fail >= 0:
        t = time.astype(float)
        m[(t >= (t_fail - fail_gap)) & (t <= (t_fail + fail_gap))] = 1.0
    return m

def copy_episode_with_labels(src_grp, dst_file, ep_idx,
                             delay_s=0.0, min_dwell_s=0.2,
    ...
    # post-fail 掩码（不受 dwell）
    postfail_mask = make_postfail_mask(time, regime_id, t_fail)

    # 可选：边界屏蔽掩码
    fail_gap_mask = make_fail_gap_mask(time, regime_id, t_fail, fail_gap=fail_gap)
    ...
    lab.create_dataset("postfail_mask",        data=postfail_mask, compression="gzip")
    if fail_gap > 0.0:
        lab.create_dataset("fail_gap_mask",    data=fail_gap_mask, compression="gzip")
    ...
    lab.attrs["fail_gap"]       = float(fail_gap)
...
def main():
    ...
    ap.add_argument("--in",  dest="in_path",
                    ...
                    # default="/home/xukai/ros2_ws/src/eeuv_sim/scripts/data_collector/data/rov_data_regimes_thrust1data10_8.hdf5",
                    default="/home/xukai/ros2_ws/src/eeuv_sim/scripts/data_collector/data/rov_data_regimes_thrust1data10_18.hdf5",
                    help="input HDF5 path")
    ap.add_argument("--out", dest="out_path", default="", help="output HDF5 path (default: *_labeled.hdf5 next to input)")
    ap.add_argument("--delay", type=float, default=0.0, help="failure label delay (s), default 0")
    ap.add_argument("--dwell", type=float, default=0.0, help="min dwell duration (s), default 0.2")
    ...
    ap.add_argument("--fail_gap", type=float, default=0.0, help="mask steps around t_fail within +/- fail_gap seconds")
    args = ap.parse_args()
    ...
    with h5py.File(in_path, "r") as fin, h5py.File(out_path, "w") as fout:
        ...
        for k in ep_keys:
            ...
            copy_episode_with_labels(
                ...
                fail_gap=args.fail_gap
            )
        fout.attrs["source_file"] = os.path.abspath(in_path)
        fout.attrs["label_maker"] = "label_make.py"
        fout.attrs["version"]     = "v1.1"
        fout.attrs["notes"]       = "Original data + labels/* per episode; added postfail_mask; default delay=0"
